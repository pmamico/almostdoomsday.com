...
const [template, events, clock] = await Promise.all([
  read("site/template.html"),
  read("data/close-calls.json"),
  read("data/doomsday-clock.json")
]);
...
const html = template
  .replace("__EVENTS__", JSON.stringify(parsedEvents))
  .replace("__CLOCK__", JSON.stringify(JSON.parse(clock)));
